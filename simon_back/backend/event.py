...
from backend.models import Medication, Event, EventAssistance
...
@require_http_methods(["GET"])
def get_by_patient(request: HttpRequest) -> JsonResponse:
    events_assistance = EventAssistance.objects.values_list('event', flat=True).filter(Q(user__id=request.GET.get("user_id")))
